...
def validate() -> dict:
            ...
            for market, windows in MARKET_WINDOWS.items():
                for window in windows:
                    admission=write_snapshot(
                        controlled_archive, market=market, window=window,
                        effective_trading_date="2026-07-17", generated_at="2026-07-17T13:09:00+08:00",
                        source_payload={"market":market,"window":window,"runtime_provenance":"scheduled_production","fixture":False,"validator":False,"validation_only":False,"cards":[{"stock_id":"2330" if market == "TW" else "AAPL","action":"觀察"}]},
                        status="completed", run_kind="scheduled", run_id=f"controlled-ai183-{market}-{window}", rebuild_routes=False,
    ...
    controlled_ok=all(row.get("written") is True for row in admissions) and controlled_sync.get("status")=="verified" and provenance["delivery_result"]=="dry_run_not_sent"
