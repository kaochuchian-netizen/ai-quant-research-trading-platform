...
ART=ROOT/'templates/four_window_dashboard_runtime_data.example.json'
HTML=ROOT/'templates/four_window_dashboard_route_preview.example.html'
DOCS=[ROOT/'docs/four_window_dashboard_runtime_data_binding_v1.md',ROOT/'docs/runbooks/four_window_dashboard_runtime_data_binding_runbook.md']
...
REQ_HTML=['四時段 AI 決策儀表板','今日決策摘要','目前追蹤標的','預測資料狀態','資料新鮮度','資料來源可信度','技術檢查 / Debug','資料待接','資料過期']
...
def allowed_path(p): return any(p==a or p.startswith(a) for a in ALLOWED)
def scan(paths):
    hits=[]
    for p in paths:
        if p.exists():
            txt=p.read_text(encoding='utf-8',errors='ignore')[:300000]
...
def validate_art(data,errors):
    ...
    if data.get('task_id')!='AI-DEV-147': errors.append('task_id mismatch')
    for key in ['data_binding_mode','global_freshness_status','latest_data_timestamp','dashboard_public_url','stock_universe','latest_reports','four_windows','prediction_fields','source_quality','freshness_checks','missing_data','inspected_sources','safety']:
    ...
    if not data.get('freshness_checks'): errors.append('freshness_checks missing')
    if not data.get('missing_data'): errors.append('missing_data must explicitly label unavailable fields')
    ...
    if not data.get('stock_universe'): errors.append('stock_universe section or missing state required')
    if not data.get('prediction_fields'): errors.append('prediction_fields section or missing state required')
    if not data.get('source_quality'): errors.append('source_quality section required')
    for src in data.get('inspected_sources',[]):
        if isinstance(src,dict):
            p=src.get('path','')
            if p and not allowed_path(str(p)): errors.append(f'inspected source not allowlisted: {p}')
            if src.get('secret_pattern_hits'): errors.append(f'secret-like hit in inspected source: {p}')
    safety=data.get('safety',{}) if isinstance(data.get('safety'),dict) else {}
    expected_false=['external_api_called','secrets_read','db_write','scheduler_modified','external_notification_sent','production_pipeline_executed','python_main_executed','trading_or_order_executed','production_rating_action_confidence_weight_mutated','formal_delivery_behavior_changed','fabricated_market_data']
...
def main():
    ...
    hits=scan([ART,HTML,*DOCS])
    if hits: errors += [f'secret pattern hit: {h}' for h in hits]
    res={'ok':not errors,'task_id':'AI-DEV-147','schema_version':'four_window_dashboard_runtime_data_binding_validation_v1','published_mode':args.published,'errors':errors,'summary':{'global_freshness_status':data.get('global_freshness_status'),'stock_universe_count':len([s for s in data.get('stock_universe',[]) if isinstance(s,dict) and s.get('stock_id')]),'missing_data_count':len(data.get('missing_data',[])) if isinstance(data.get('missing_data'),list) else None,'secret_pattern_hits':len(hits),'published_summary':pub}}
