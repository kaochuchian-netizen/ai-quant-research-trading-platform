...
def inspect_db():
    ...
    if 'analysis_results' in res['tables']:
        ...
        cur.execute('select '+', '.join('a.'+c for c in select_cols)+' from analysis_results a join (select stock_id, max(created_at) latest_created_at from analysis_results group by stock_id) latest on latest.stock_id=a.stock_id and latest.latest_created_at=a.created_at order by a.stock_id limit 100')
        latest=[]
...
def missing_prediction_fields():
    return [{'field':k,'label':label,'data_state':'missing','message':f'資料待接：未找到具 timestamp 的正式 {label} 欄位','source_timestamp':None,'freshness_status':'missing'} for k,label in [('same_day_high_low','今日最高價預測 / 今日最低價預測'),('next_day_high_low','隔天最高價預測 / 隔天最低價預測'),('one_month_trend','未來 1 個月走勢'),('three_month_trend','未來 3 個月走勢'),('confidence','信心分數'),('rationale','主要依據')]]
