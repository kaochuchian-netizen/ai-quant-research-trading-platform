...
def validate():
 ...
 trade=q["trade_outcome_counts"]
 checks={"prediction_traceable":all(x.get("prediction_source_window")=="pre_open_0700" and x.get("prediction_snapshot_id") for x in c),"actual_ohlc":sum(x.get("actual_status")=="complete" for x in c)==8,"canonical_outcomes":sum(oc.values())==9 and set(oc)=={"hit","fail","not_triggered","no_trade","pending"},"mfe_mae":all("mfe_pct" in x and "mae_pct" in x and "mfe" in x and "mae" in x for x in c),"review_semantics":q["completed_review_count"]+q["open_at_close_count"]+q["pending_review_count"]==q["review_universe_count"]==9,"not_all_pending":trade["pending_evidence"]<9,"open_at_close_explicit":"open_at_close" in trade,"no_none_range":all(x.get("review_snapshot",{}).get("actual_range")!={"low":None,"high":None} for x in c if x.get("actual_status")=="complete")}
