...
def main() -> int:
    ...
    checks = {
        ...
        "secondary_context_contracts": all(key in nvda_origin["context_contracts"] for key in ("news", "macro", "options", "analyst", "insider")) and all(nvda_origin["context_contracts"][key]["status"] in {"AVAILABLE", "PARTIAL", "CONTRADICTORY", "STALE", "MISSING", "FAILED", "NOT_APPLICABLE"} for key in ("news", "macro", "options", "analyst", "insider")),
        ...
        "carryforward": bool(post["next_session_carryforward"]["carryforward_reason"]) and isinstance(post["next_session_carryforward"]["missing_critical_sources"], list),
