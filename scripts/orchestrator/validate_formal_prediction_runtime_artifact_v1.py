...
CONFIDENCE_LEVELS = {"low", "medium", "medium_high", "insufficient_data", None}
...
def validate_doc(data: dict[str, Any], label: str) -> list[str]:
    ...
    if data.get("artifact_type") != "formal_prediction_runtime": errors.append(f"{label}: artifact_type must be formal_prediction_runtime")
    if data.get("runtime_window") != "pre_open_0700": errors.append(f"{label}: runtime_window must be pre_open_0700")
    stocks = data.get("stocks")
    if not isinstance(stocks, list) or not stocks:
        errors.append(f"{label}: stocks must be non-empty list"); return errors
    for i, stock in enumerate(stocks):
        if not isinstance(stock, dict):
            errors.append(f"{label}: stock {i} must be object"); continue
        ...
        for key in NUMERIC_FIELDS:
            ...
            if value is None:
                if key not in stock.get("missing_fields", []) and not stock.get("insufficient_data_reasons"):
                    errors.append(f"{label}: null {key} requires missing_fields or insufficient_data_reasons")
            elif not finite_number(value) or float(value) <= 0 or float(value) in {0.0, -1.0}:
                errors.append(f"{label}: {key} must be null or positive non-placeholder number")
        if finite_number(stock.get("same_day_high_prediction")) and finite_number(stock.get("same_day_low_prediction")) and stock["same_day_high_prediction"] < stock["same_day_low_prediction"]:
            errors.append(f"{label}: same_day_high_prediction must be >= same_day_low_prediction")
        if finite_number(stock.get("next_day_high_prediction")) and finite_number(stock.get("next_day_low_prediction")) and stock["next_day_high_prediction"] < stock["next_day_low_prediction"]:
            errors.append(f"{label}: next_day_high_prediction must be >= next_day_low_prediction")
        score = stock.get("confidence_score")
        if score is not None and (not finite_number(score) or float(score) < 0 or float(score) > 75):
            errors.append(f"{label}: confidence_score must be null or 0-75")
        if stock.get("confidence_level") not in CONFIDENCE_LEVELS:
            errors.append(f"{label}: confidence_level invalid")
        for key in ("one_month_trend", "three_month_trend"):
            if stock.get(key) not in TREND_VALUES:
                errors.append(f"{label}: {key} invalid trend value")
        if stock.get("model_version") != "deterministic_baseline_v1": errors.append(f"{label}: model_version must be deterministic_baseline_v1")
...
def main() -> int:
    ...
    if runtime is not None:
        ...
        if runtime.get("safety", {}).get("fabricated_forecast_values") is not False: errors.append("runtime: fabricated_forecast_values must be false")
    result = {"ok": not errors, "errors": errors, "template_checked": template is not None, "runtime_checked": runtime_checked, "artifact": str(args.artifact), "summary": {"artifact_type": "formal_prediction_runtime", "runtime_window": "pre_open_0700", "forecast_value_count": runtime.get("forecast_value_count") if runtime else None, "secret_pattern_hits": 0 if not errors else None}}
