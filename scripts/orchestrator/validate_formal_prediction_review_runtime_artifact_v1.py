...
REVIEW_VALUE_FIELDS = ["actual_high", "actual_low", "actual_close", "direction_result", "high_low_forecast_error", "hit_miss_status", "seven_day_hit_rate", "confidence_calibration", "factor_effectiveness", "recommendation_for_improvement"]
...
def validate_doc(data: dict[str, Any], label: str) -> list[str]:
    ...
    if data.get("artifact_type") != "formal_prediction_review_runtime":
        errors.append(f"{label}: artifact_type must be formal_prediction_review_runtime")
    if data.get("runtime_window") != "prediction_review_1500":
        errors.append(f"{label}: runtime_window must be prediction_review_1500")
    if data.get("review_window_days") != 7:
        errors.append(f"{label}: review_window_days must be 7")
    ...
    for i, stock in enumerate(stocks):
        ...
        if hit_rate is not None and (not finite_number(hit_rate) or float(hit_rate) < 0 or float(hit_rate) > 100):
            errors.append(f"{label}: seven_day_hit_rate must be null or 0-100")
        for key in REVIEW_VALUE_FIELDS:
            if stock.get(key) is None and (key not in stock.get("missing_fields", []) or not stock.get("insufficient_data_reasons")):
...
def main() -> int:
    ...
    result = {"ok": not errors, "errors": errors, "template_checked": template is not None, "runtime_checked": runtime_checked, "artifact": str(args.artifact), "summary": {"artifact_type": "formal_prediction_review_runtime", "runtime_window": "prediction_review_1500", "secret_pattern_hits": 0 if not errors else None}}
