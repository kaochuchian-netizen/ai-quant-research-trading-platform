...
- Parent IPC deadlines independently bound page/script/readiness operations as well as the overall budget. Cleanup resources are each attempted even when another resource cleanup fails. Worker-only alarm bounds lifetime even on caller death; EOF and SIGTERM also enter cleanup. No caller signal handlers are changed.
...
Linux integration uses **fake Selenium** with a real service → detached-grandchild Python process tree. The success fixture exits/reaps its service before cleanup, explicitly requiring subreaper adoption rather than a live-service-only tree walk. It verifies success, partial construction, constructor hang, navigation failure, overall timeout, quit failure/hang, cancellation, parent EOF and idle expiry; every recorded child PID must disappear, the worker must exit, the temporary profile must disappear, the lock must be reusable and an unrelated process must survive. No real Chrome smoke test or production pipeline is run.

Local macOS: 26 unit tests PASS; ten Linux-only cases require Linux CI and are reported as skipped locally. Existing CNYES materiality validator 61/61, AI-DEV-248 24/24, AI-DEV-249 20/20, US research regression 34 checks passed. Local full branch gate was executed: existing visual validators assume Linux `/tmp` paths and fail on macOS; the existing AI-DEV-237 multiprocessing PID-lock test also observed two sequential winners under macOS spawn. No gates were weakened or unrelated validators changed. Linux CI run 35753160464 passed the complete branch gate, source inventory and governance on the initial implementation (34/34 lifecycle tests, zero skips). Two additional cleanup/operation-deadline tests were subsequently added and final-head CI is tracked in the PR; no claim of natural production verification is made.
