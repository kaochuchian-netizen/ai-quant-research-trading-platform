...
def _normalize_selected(item: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "publisher": item.get("publisher") or "原始來源未解析",
        "publisher_resolution_status": item.get("publisher_resolution_status") or ("resolved" if item.get("publisher") else "unresolved"),
        "discovery_channel": item.get("discovery_channel") or "YAHOO_FINANCE",
