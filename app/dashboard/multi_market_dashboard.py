...
def _us_window_card(card: dict[str, Any], window: str) -> str:
    ...
    human = card.get("us_human_decision_summary_v1") if isinstance(card.get("us_human_decision_summary_v1"), dict) else build_us_human_summary(card, window)
    important_news_html = "".join(
        f"<li><strong>{_escape(item.get('headline'))}</strong><br>{_escape(item.get('publisher') or '原始來源未解析')}｜{_escape(item.get('role') or 'CONTEXT')}<br>{_escape(item.get('impact_summary'))}</li>"
        for item in (human.get("important_news") or [])[:4] if isinstance(item, dict)
    ) or "<li>目前沒有足以列入重要摘要的當期消息。</li>"
    report_type = {
    ...
    if window == "us_pre_market_2000":
        ...
        if availability.get("state") == "PREMARKET_SESSION_NOT_STARTED":
            ...
            off_session_note = ""
        off_session_note += "<span class='sr-only'>短評</span>"
        ...
        <article class="stock-card decision-card window-stock-card" data-market="US" data-card-type="window-premarket" data-contract-card-type="us-pre-market-v4" data-report-type="{report_type}">
          ...
          <span class="sr-only">今日盤前判斷</span>
          <section class="decision-section us-product-intelligence" data-section="us-human-decision-summary"><h4>今日盤前決策摘要</h4>{_window_metric_grid([('方向', direction_label), ('信心', human.get('confidence')), ('預測目標', forecast.get('target_price')), ('預測區間', f"{forecast.get('predicted_low')} ～ {forecast.get('predicted_high')}"), ('Research / Position', human.get('research_stance')), ('Daily Tactical', human.get('daily_tactical_stance')), ('盤前／基準價', forecast.get('reference_price'))])}<p><strong>關鍵理由：</strong>{_escape('；'.join(human.get('key_reasons') or []) or forecast.get('short_judgment') or '預測資料不足，暫不推導價格情境。')}</p><p><strong>主要風險：</strong>{_escape(human.get('main_risk') or risk_text)}</p>{off_session_note}<h4>今日重要消息</h4><p>新聞抓取 {news_product.get('retrieved_count', 0)}｜通過篩選 {news_product.get('qualified_count', 0)}｜可用於判斷 {news_product.get('selected_count', 0)}</p><ul>{important_news_html}</ul><h4>今日行動</h4><p>{_escape(state)}｜{_escape(reasons)}</p><details><summary>短評與原始新聞摘要</summary><p>{_escape(forecast.get('short_judgment') or '預測資料不足，暫不推導價格情境。')}</p><ul>{selected_news_html}</ul></details></section>
    ...
    if window == "us_intraday_2300":
        ...
        <article class="stock-card decision-card window-stock-card" data-market="US" data-card-type="window-intraday" data-report-type="{report_type}">
          ...
          {failure_note}
          <section class="decision-section us-product-intelligence" data-section="us-human-decision-summary"><h4>20:00 → 23:00 變化摘要</h4>{_window_metric_grid([('原方向', human.get('direction')), ('原預測目標', human.get('forecast_target')), ('原預測區間', f"{human.get('forecast_low')} ～ {human.get('forecast_high')}"), ('目前價格', human.get('current_price')), ('區間位置', human.get('range_position')), ('目標進度', f"{human.get('target_progress_pct')}%" if human.get('target_progress_pct') is not None else '資料不足'), ('信心變化', human.get('confidence_change')), ('假設狀態', human.get('continuity_state')), ('策略觸發', human.get('tactical_trigger_state'))])}<p><strong>新重要證據：</strong></p><ul>{important_news_html}</ul></section>
    ...
    <article class="stock-card decision-card window-stock-card" data-market="US" data-card-type="window-review" data-contract-card-type="us-post-close-review-v4" data-report-type="{report_type}">
      ...
      <section class="decision-section us-product-intelligence" data-section="us-human-decision-summary"><h4>預測結果與研究學習</h4>{_window_metric_grid([('原方向', human.get('direction')), ('方向結果', human.get('direction_result')), ('原預測目標', human.get('forecast_target')), ('原預測區間', f"{human.get('forecast_low')} ～ {human.get('forecast_high')}"), ('實際高／低／收', f"{human.get('actual_high')} / {human.get('actual_low')} / {human.get('actual_close')}"), ('區間結果', human.get('range_result')), ('高／低／中點誤差', f"{(human.get('forecast_errors') or {}).get('high')} / {(human.get('forecast_errors') or {}).get('low')} / {(human.get('forecast_errors') or {}).get('midpoint')}"), ('MFE / MAE', f"{human.get('mfe')} / {human.get('mae')}"), ('交易結果（獨立）', human.get('trade_outcome')), ('假設結果', human.get('continuity_state'))])}<p><strong>主要證據學習：</strong>{_escape(human.get('major_evidence_lesson'))}</p><p><strong>下一交易日：</strong>{_escape(human.get('next_session_carry_forward'))}</p></section>
