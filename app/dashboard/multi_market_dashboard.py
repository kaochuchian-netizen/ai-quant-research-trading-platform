...
def _us_window_card(card: dict[str, Any], window: str) -> str:
    ...
    sec_display = "｜".join(str(value) for value in (source_sec.get("form"), source_sec.get("filing_date"), source_sec.get("item") or source_sec.get("materiality")) if value) or "尚未取得"
...
def _tw_pre_close_card(card: dict[str, Any]) -> str:
    ...
    <article class="stock-card decision-card window-stock-card" data-market="TW" data-card-type="window-snapshot" data-report-type="pre-close-snapshot">
      ...
      <section class="decision-section" data-section="pre-close-summary"><h4>收盤前摘要</h4>{_window_metric_grid([('目前價格', safe_public_text(card.get('current_price'))), ('行情時間', format_timestamp(card.get('market_data_as_of'), reference_value=card.get('fetched_at'))), ('觸發狀態', localize_tw_value(card.get('trigger_status'))), ('留倉決策', action)])}</section>
...
def _tw_post_close_card(card: dict[str, Any]) -> str:
    ...
    <article class="stock-card decision-card window-stock-card" data-market="TW" data-card-type="window-review" data-report-type="post-close-review">
      ...
      <section class="decision-section" data-section="prediction-review"><h4>預測與交易結果</h4>{_window_metric_grid([('預測區間結果', prediction_result), ('交易結果', trade_detail), ('實際低 / 高', f"{safe_public_text(card.get('actual_low'), missing='不適用')} / {safe_public_text(card.get('actual_high'), missing='不適用')}"), ('明日行動', next_action_for_outcome(card.get('canonical_outcome')))])}</section>
      ...
      <section class="decision-section" data-section="mfe-mae"><h4>交易歷程</h4>{_window_metric_grid([('最大有利變動', f"{float(mfe.get('pct')):+.2f}%" if isinstance(mfe.get('pct'), (int,float)) else '不適用'), ('最大不利變動', f"{float(mae.get('pct')):+.2f}%" if isinstance(mae.get('pct'), (int,float)) else '不適用'), ('基準', f"首次進場觸發價 {safe_public_text(mfe.get('reference_price'), missing='尚未取得')}"), ('行情解析度', '5 分鐘' if mfe.get('resolution') == 'minute_5' else '日內高低價'), ('今日決策歷程', timeline_text or '本批次尚未保存完整 identity chain'), format_market_time(card.get('market_data_as_of'), source_timezone=card.get('source_timezone'), time_kind=card.get('source_record_time_kind'), reference_value=card.get('fetched_at'), trading_date=card.get('trading_date'), market='TW')])}</section>
    </article>
    """
...
def _tw_pre_open_structured_card(card: dict[str, Any]) -> str:
    ...
    if actionable:
        ...
        plan = _window_metric_grid([('目前狀態', '暫不交易'), ('原因', safe_public_text(card.get('do_not_trade_reason') or card.get('risk_summary'))), ('重新評估條件', '資料完整且策略條件重新成立')])
    else:
        zone = f"{card.get('entry_low')}–{card.get('entry_high')}" if card.get('entry_low') is not None else '尚未取得'
        plan = _window_metric_grid([('目前狀態', '觀察等待'), ('觀察區間', zone), ('等待條件', safe_public_text(card.get('entry_condition'))), ('正式交易計畫', '暫不建立'), ('原因', safe_public_text(card.get('risk_summary')))])
    ...
    context_metrics = [('市場環境', safe_public_text(card.get('market_context'))), ('技術', safe_public_text(card.get('technical_summary')))]
    ...
    risk_metrics = [('追價風險', localize_enum(card.get('chase_risk'))), ('風險摘要', safe_public_text(card.get('risk_summary'))), ('不交易原因', safe_public_text(card.get('do_not_trade_reason'), missing='不適用'))]
    ...
    <article class="stock-card decision-card window-stock-card tw-pre-open-structured-card"
      ...
      <details class="decision-details"><summary>資料新鮮度與缺口</summary><div class="decision-details__body">{_window_metric_grid([('技術資料', freshness.get('technical_as_of') or '前一交易日收盤'), ('報告產生', freshness.get('report_generated_at') or '本批次尚未取得'), ('資料缺口', '、'.join(gaps) or '無')])}</div></details>
    </article>
    """
...
def render_tw_window_report(window: str, artifact: dict[str, Any] | None = None) -> str:
    ...
    if window == "pre_open_0700":
        ...
        <section class="section window-report-section" data-market="TW" data-window="{_escape(window)}" data-report-type="pre-open-decision">
          ...
          <p class="decision-note">市場基調：{_escape('偏保守' if summary.get('market_bias') == 'cautious' else '中性')}｜信心：{_escape('偏低' if summary.get('market_bias_confidence') == 'low' else '中')}</p>
          ...
          <p class="decision-note">資料覆蓋：{_escape('｜'.join(f"{label} {((summary.get('coverage') or {}).get(key) or {}).get('available', 0)}/{((summary.get('coverage') or {}).get(key) or {}).get('total', len(cards))}" for key,label in [('technical','技術'),('overnight','ADR／隔夜'),('chip','籌碼'),('news','新聞'),('gap','Gap'),('event_risk','事件風險')]))}</p>
    ...
    if window == "post_close_1500":
        ...
        structured_summary = _window_metric_grid([
            ...
            ("正式交易計畫", plans.get("active", 0) + plans.get("watch", 0)),
