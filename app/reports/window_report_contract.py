...
CONTRACTS: dict[tuple[str, str], WindowReportContract] = {
    ("TW", "pre_open_0700"): WindowReportContract(
        ...
        ("今日盤前重點", "市場環境", "今日可觀察標的", "高風險標的", "研究摘要", "每日短線策略摘要", "今日預測區間", "進場區 / 停損 / 目標價", "主要新聞與事件", "資料完整度"),
    ...
    ("TW", "intraday_1305"): WindowReportContract(
        ...
        ("盤中變化摘要", "與盤前相比的變化", "已觸發 setup", "尚未觸發 setup", "接近 stop 的標的", "接近 target 的標的", "量價確認狀態", "盤中風險變化"),
    ...
    ("TW", "pre_close_1335"): WindowReportContract(
        ...
        ("收盤前快照", "尾盤風險", "是否避免追高", "接近目標 / 接近停損", "當日 setup 狀態", "明日初步觀察", "留倉 / 觀望提示"),
    ...
    ("TW", "post_close_1500"): WindowReportContract(
        ...
        ("盤後檢討", "今日預測 vs 實際", "進場是否觸發", "目標是否觸發", "停損是否觸發", "成功 / 失敗 / 未觸發 / 無交易", "False Breakout", "7 日滾動檢討", "明日觀察清單"),
    ...
    ("US", "us_pre_market_2000"): WindowReportContract(
        ...
        ("美股盤前重點", "Premarket / gap 狀態", "SPY / QQQ / 類股脈絡", "今日 Tactical setup", "進場 / 停損 / 目標價", "財報 / SEC / 官方事件", "近期市場新聞", "高波動 / 高追價風險"),
    ...
    ("US", "us_post_close_review_0630"): WindowReportContract(
        ...
        ("美股盤後檢討", "預測檢討", "進場 / 停損 / 目標價 outcome", "成功 / 失敗 / 未觸發", "財報 / SEC / 隔夜新聞更新", "隔日觀察"),
