...
CONTRACTS: dict[tuple[str, str], WindowReportContract] = {
    ...
    ("US", "us_pre_market_2000"): WindowReportContract(
        ...
        "美股開盤前方向、預測目標與區間是什麼，哪些新聞影響今日判斷？",
        ("方向", "預測目標", "預測區間", "短評", "新聞漏斗", "Selected news", "今日行動", "Premarket / gap 狀態", "Entry / Stop / execution Target", "財報 / SEC / 官方事件", "與前一次 20:00 同時段的變化"),
        ("方向", "預測目標", "預測區間", "短評", "新聞漏斗", "Selected news", "今日行動", "Entry / Stop / execution Target"),
        ("方向分布", "預測目標與區間", "新聞抓取 / 通過 / 可用", "US Dashboard URL"),
    ...
    ("US", "us_intraday_2300"): WindowReportContract(
        ...
        "開盤後是否符合盤前預期？盤前策略是否確認？",
        ("美股盤中變化", "已確認策略", "Gap 失效", "量能確認", "仍可行動", "追價風險", "Gap 延續", "進場觸發", "目標／停損接近度", "盤中新聞／事件更新", "盤中策略調整", "與 20:00 預期的偏差"),
        ("美股盤中變化", "Gap 延續", "量能確認", "進場觸發狀態", "目標／停損接近度", "盤中新聞／事件更新", "盤中策略調整"),
        ("美股盤中摘要", "已確認策略：N", "接近目標：N", "接近停損：N", "US Dashboard URL"),
        ("完整 Research 長文", "完整財務卡片", "完整 SEC 長列表"),
        ("23:00 美股盤中", "Gap 延續", "量能確認"),
